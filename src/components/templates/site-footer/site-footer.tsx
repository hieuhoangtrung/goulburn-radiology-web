    ...
    <Box component="footer" sx={{ bgcolor: '#0B2747', color: '#C8D8EB', mt: 'auto' }}>
        ...
        <Grid container spacing={5}>
          ...
          <Grid item xs={12} md={4}>
            <Box sx={{ mb: 2 }}>
              ...
              <Box sx={{ color: '#4A90D9', fontWeight: 600, fontSize: '1.3rem', letterSpacing: '0.05em', textTransform: 'uppercase' }}>
                Medical Imaging
              </Box>
            </Box>
            <Typography sx={{ fontSize: '1.45rem', lineHeight: 1.7, color: '#A8BDD0', mb: 3 }}>
              Providing expert diagnostic imaging services to Goulburn and the Southern Tablelands since 1985. Committed to quality, compassion and clinical excellence.
            </Typography>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
              <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1.2 }}>
                <LocationOnIcon sx={{ fontSize: '1.8rem', color: '#4A90D9', mt: 0.3, flexShrink: 0 }} />
                <Typography sx={{ fontSize: '1.4rem', color: '#A8BDD0' }}>
                  {BUSINESS.addressStreet}<br />
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2 }}>
                ...
                <a href={`tel:${BUSINESS.phone.replace(/\s/g, '')}`} style={{ color: '#A8BDD0', textDecoration: 'none', fontSize: '1.4rem' }}>
                  {BUSINESS.phone}
                </a>
              </Box>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2 }}>
                <EmailIcon sx={{ fontSize: '1.8rem', color: '#4A90D9', flexShrink: 0 }} />
                <a href={`mailto:${BUSINESS.email}`} style={{ color: '#A8BDD0', textDecoration: 'none', fontSize: '1.4rem' }}>
                  {BUSINESS.email}
                </a>
              </Box>
            </Box>
          ...
          <Grid item xs={12} sm={6} md={2.5}>
            <Typography sx={{ color: '#fff', fontWeight: 700, fontSize: '1.6rem', mb: 2, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              Services
            </Typography>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              {SERVICES.map((s: import('@src/lib/content').Service) => (
                <Link key={s.slug} href={`/services/${s.slug}`} style={{ color: '#A8BDD0', textDecoration: 'none', fontSize: '1.4rem' }}>
          ...
          <Grid item xs={12} sm={6} md={2}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              {NAV_LINKS.map((link: { label: string; href: string }) => (
                <Link key={link.href} href={link.href} style={{ color: '#A8BDD0', textDecoration: 'none', fontSize: '1.4rem' }}>
          ...
          <Grid item xs={12} md={3.5}>
            ...
            <Box sx={{ mt: 3, p: 2, bgcolor: 'rgba(74,144,217,0.15)', borderRadius: '8px', border: '1px solid rgba(74,144,217,0.3)' }}>
              <Typography sx={{ fontSize: '1.35rem', color: '#A8BDD0' }}>
                ...
                After-hours emergency services are available through Goulburn Base Hospital.
              </Typography>
            </Box>
          </Grid>
        </Grid>
      ...
      <Container maxWidth="lg" sx={{ py: 2.5 }}>
        <Box sx={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: 2 }}>
          <Typography sx={{ fontSize: '1.3rem', color: '#6B8299' }}>
            © {year} {META.siteName}. All rights reserved. ABN 12 345 678 901.
          </Typography>
          <Box sx={{ display: 'flex', gap: 3, fontSize: '1.3rem' }}>
            <Link href="/privacy" style={{ color: '#6B8299', textDecoration: 'none' }}>Privacy Policy</Link>
            <Link href="/contact" style={{ color: '#6B8299', textDecoration: 'none' }}>Contact</Link>
          </Box>
        </Box>
      </Container>
    </Box>
